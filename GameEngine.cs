...
public class GameEngine
{
    ...
    public void ExecuteMove(Player player, Move move)
    {
        ...
        {
            Console.WriteLine($"[{player.Color}] player moves piece {move.Piece.Id} to the starting point.");
            Console.WriteLine($"[{player.Color}] player now has {player.PiecesOnBoard() + 1}/4 on pieces on the board and {player.PiecesInBase() - 1}/4 pieces on the base.");
        ...
        {
            Console.WriteLine($"[{player.Color}] piece {move.Piece.Id} is movement-restricted and has rolled three consecutively. Teleporting piece {move.Piece.Id} to base.");
        }
        else if (move.MoveDistance > 0)
        {
            foreach (var p in piecesToMove)
            {
                Console.WriteLine($"[{player.Color}] moves piece {p.Id} from location {p.Position} to {move.TargetPosition} by {move.MoveDistance} units in {move.MoveDirection.ToString().ToLower()} direction.");
            }
        }
        ...
        if (move.DoesCapture)
        {
            ...
            {
                Console.WriteLine($"[{player.Color}] piece {move.Piece.Id} lands on square {move.TargetPosition}, captures [{captured.Color}] piece {captured.Id}, and returns it to the base.");
                captured.Reset();
                
                // Increment capture count for all participating in capture (Rule T-8)
                foreach (var p in piecesToMove) p.CapturedPiecesCount++;
            }
            Console.WriteLine($"[{player.Color}] player now has {player.PiecesOnBoard()}/4 on pieces on the board and {player.PiecesInBase()}/4 pieces on the base.");
        }
