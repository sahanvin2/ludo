...
public class MysteryCellManager
{
    ...
    private void Spawn(List<Piece> allPieces)
    {
        ...
        if (available.Count > 0)
        {
            ...
            GameEngine.Log($"Mystery Cells : A mystery cell has spawned in location {CurrentLocation} and will be at this location for the next four rounds.");
        }
    }
    ...
    public void TeleportPiece(Piece p)
    {
        ...
        GameEngine.Log($"[{p.Color}] piece {p.Id} lands on Mystery Cell and is teleported to {locName}.");
        ...
        switch (choice)
        {
            case 1:
                ...
                if (rand.Next(2) == 0)
                {
                    ...
                    GameEngine.Log($"[{p.Color}] piece {p.Id} feels energized, and movement speed doubles.");
                }
                else
                {
                    p.Status = StatusEffect.Sick;
                    p.EffectRoundsRemaining = 4;
                    GameEngine.Log($"[{p.Color}] piece {p.Id} feels sick, and movement speed halves.");
                }
                break;
            case 2:
                p.Position = 27;
                ...
                GameEngine.Log($"[{p.Color}] piece {p.Id} attends briefing and cannot move for four rounds.");
                break;
            case 3:
                p.Position = 46;
                if (p.Direction == Direction.Clockwise)
                {
                    ...
                    GameEngine.Log($"The [{p.Color}] piece {p.Id}, which was moving clockwise, has changed to moving counterclockwise.");
                }
                else
                {
                    GameEngine.Log($"The [{p.Color}] piece {p.Id} is moving in a counterclockwise direction. Teleporting to Beta from Gamma.");
                    p.Position = 27;
                    p.Status = StatusEffect.Briefing;
                    p.EffectRoundsRemaining = 4;
                    p.ConsecutiveThreesRolled = 0;
